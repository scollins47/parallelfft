...
void bit_reverse_reorder(struct complex *W, int N)
{
    ...
    // Build array of j values. MUST BE DONE SERIALLY
    int *j_array = malloc(sizeof(int) * (size_t)N);
    for (i = 0; i < N; i++)
    {
        j = 0;
        for (k = 0; k < bits; k++)
    ...
    // copy array
    struct complex *original_W = malloc(sizeof(struct complex) * (size_t)N);
    if (original_W == NULL) {
        fprintf(stderr, "Out of memory!\n");
        exit(1);
    }
    memcpy(original_W, W, sizeof(struct complex) * (size_t)N);

    // can now be done in parallel
# pragma omp parallel for
    for (i = 0; i < N; i++) {
        int k = j_array[i];
        W[k].r = original_W[i].r;
        W[k].i = original_W[i].i;
...
void radix2(struct complex *data, int N)
{
    ...
    /** Don't recurse if we're down to one butterfly */
    // No longer needs a loop. Can just be accomplished with OMP tasks
    if (N2 != 1)
    {
#pragma omp task
        radix2(&data[N2], n2);
...
#pragma omp single nowait
    ...
    printf("rad2: %8.4f\treverse_reorder: %8.4f\n", GET_TIMER(RAD2), GET_TIMER(BIT));
    printf("rad2 combined: %8.4f\n", GET_TIMER(RAD2) + GET_TIMER(BIT));
